...
public class PlayerAttackB : MonoBehaviour
{
    ...
    void PerformAttack()
    {
        ...
        if (Physics.Raycast(ray, out hit, 100f, enemyLayer))
        {
            ...
            EnemyReactionB reaction = hit.collider.GetComponent<EnemyReactionB>();
            if (reaction)
                reaction.ReactToHit();
        }
        else
        {
            // optional: feedback when missing
            Debug.Log("Gave the finger to the void!");
        }
    }
}
